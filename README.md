...
- **Four extraction paths**: `extraction_mode="docling_text"` (**recommended default**) uses Docling's neural layout analysis for extraction, then the app's own configurable chunker (`KNOWLEDGE_CHUNKING_STRATEGY`, default `agentic`). `extraction_mode="docling"` uses Docling extraction *and* its own `HybridChunker` instead. `extraction_mode="text"` (legacy) uses pymupdf4llm with corpus cleaning rules. `extraction_mode="vision"` renders pages via a local Ollama vision model (`blaifa/Nanonets-OCR-s`) — benchmarked in [spec 015](specs/015-vision-extraction-benchmark/) against `docling_text` on the same 118-question gold-standard set and **not adopted as default**: despite a slight retrieval-recall edge, the LLM-judge answer-quality aggregate regressed −19.1pp (0.838 → 0.647), and a fidelity spot-check found vision reproducing the drop-cap defect it was meant to fix plus a same-content table regression versus `docling_text`. Switching extraction mode requires re-ingestion; `extraction_mode` is stored per-chunk in metadata.
