...
- Q: Which session persistence mechanism should be used given Gradio's tab-local state limitation? → A: Gradio built-in auth (initial phase) — Gradio's `auth` parameter with a callable validating credentials against the application database. FastAPI deferred to a future phase. Registration is served via a companion unauthenticated Gradio interface at a separate path (e.g. `/register`).
  ...
  > **Implementation Decision (2026-06-19)**: The plan was revised during implementation. Instead of Gradio's built-in `auth=` callable + a separate `/register` Gradio app, the final implementation uses a **single Gradio app** with a custom `auth_col` panel (`pages/auth.py`) containing Sign In and Create Account tabs. `user_state: gr.State(UserInfo | None)` carries the authenticated identity per browser tab — Gradio guarantees tab isolation for `gr.State`. This approach was chosen because: (1) it allows auto-login immediately after registration (setting `user_state` triggers navigation without a page reload); (2) it eliminates cross-path session complexity between two Gradio instances; (3) the `registration.py` standalone companion (`create_registration_app()`) was created but remains unmounted — kept as a future opt-in standalone registration endpoint. The `make_auth_callable` function in `services/auth.py` is retained for possible future use. `gr.Request` identity is replaced by `user_state` as the source of truth for the current user inside handlers.
