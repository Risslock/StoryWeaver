...
- [X] T020 [US2] Run Quickstart Scenario 4 end-to-end: `eval_runner.py` + `judge_runner.py --summary` with `HYBRID_SEARCH_ENABLED=false`, then again with `HYBRID_SEARCH_ENABLED=true`; confirm both produce complete `judge_aggregate` summaries and that the hybrid run's `exact_term` Recall@10/MRR exceeds the baseline by more than the 2-percentage-point threshold from spec.md's Clarifications, with no standard category regressing by more than 2 points (depends on T009, T018, T019). **RUN 2026-07-07** (run_id `baseline-014`/`hybrid-014`, campaign `6e5fc0d4-1fea-4d01-9aca-b2af32df4f19`): both produced complete `judge_aggregate` summaries (baseline 111/118 scored, hybrid 116/118 scored — coverage within the historical 94-98% parse-error band, not a regression indicator). Results: `exact_term` judge_aggregate improves well past threshold (0.864→0.916, +5.2pp), consistent with the retrieval-level SC-002 pass. **Global judge_aggregate regresses 0.831→0.798 (−3.3pp), exceeding the 2pp threshold** — `direct_fact` (−3.4pp), `numeric` (−5.4pp), and `relationship` (−6.1pp) all regress on judge score despite `direct_fact`/`numeric` having *improved* retrieval metrics; sub-dimension breakdown shows context_utilization (0.779→0.733) and answer_correctness (0.747→0.698) driving the drop. Working theory: lexical-only chunks lack the `headline`/`summary` metadata vector-sourced chunks carry (FTS5 stores raw text only), thinning `_rerank_repr()` for those candidates and degrading the context handed to answer synthesis even when the raw retrieved-chunk set is more complete. **Per user decision (2026-07-07): hyperparameter tuning (`HYBRID_SEARCH_KEYWORD_WEIGHT`/`_VECTOR_WEIGHT`) and prompt-level fixes are explicitly out of scope for this spec — deferred to a dedicated follow-up spec.** Feature 014 ships as the toggleable infrastructure + measurement harness; the SC-003 regression is a documented, known finding for that follow-up to address, not a blocker fixed here.
...
- [X] T023 [US3] Run Quickstart Scenario 2 end-to-end (disabled → enabled with default weights → invalid weight value) and confirm all three pass criteria (depends on T009, T021, T022). Verified 2026-07-07: disabled run Recall@10=0.907 (sanity gate ≥0.40 passes); enabled run appended a second `benchmark_results.jsonl` record with `hybrid_search_enabled: true`; `HYBRID_SEARCH_KEYWORD_WEIGHT=0` raises `ValueError` at `Settings()` construction, not at query time.
...
- [X] T027 Run all 5 Quickstart Scenarios from `specs/014-hybrid-search/quickstart.md` end-to-end and confirm SC-001 through SC-005 pass. **RESULT 2026-07-07**: All 5 scenarios run. Scenario 1 (backfill, idempotent, 780 chunks) ✅. Scenario 2 (T023, config toggle + fail-fast) ✅. Scenario 3 (exact-term recall — SC-002) ✅ +11.4pp. Scenario 4 (judge pipeline — SC-003, see T020) ⚠️ exact-term judge score passes (+5.2pp) but global judge_aggregate regresses −3.3pp, past threshold. Scenario 5 (access-level filtering with hybrid enabled) ✅ zero gm_only leakage. **Net**: SC-002, SC-004, SC-005 pass cleanly; SC-001 passes at the global retrieval-metric level but not per-category (`holistic` regresses); SC-003 fails on the global judge-score condition. Hyperparameter/prompt tuning to close this gap is deferred to a follow-up spec per user decision — feature 014 ships as the infrastructure + measurement harness that made this comparison possible in the first place.
