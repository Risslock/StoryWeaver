...
- [ ] T020 [US2] Run Quickstart Scenario 4 end-to-end: `eval_runner.py` + `judge_runner.py --summary` with `HYBRID_SEARCH_ENABLED=false`, then again with `HYBRID_SEARCH_ENABLED=true`; confirm both produce complete `judge_aggregate` summaries and that the hybrid run's `exact_term` Recall@10/MRR exceeds the baseline by more than the 2-percentage-point threshold from spec.md's Clarifications, with no standard category regressing by more than 2 points (depends on T009, T018, T019). **STATUS 2026-07-07**: Not yet run — requires `--campaign-id` and ~20-40 min of additional Ollama generation per side. Gold-standard (retrieval-only) comparison is complete: global Recall@10/MRR/nDCG all improve (+1.8pp/+7.1pp/+4.8pp) and `exact_term` improves well past threshold (+11.4pp Recall@10), but the `holistic` category regresses beyond the 2pp threshold (MRR −10.8pp, nDCG −6.9pp, Recall −4.2pp) — a real tradeoff, not noise. Awaiting a decision on whether to (a) run the judge pipeline anyway, (b) tune `HYBRID_SEARCH_KEYWORD_WEIGHT` down to reduce the holistic regression first, or (c) treat the retrieval-level result as sufficient.
...
- [ ] T027 Run all 5 Quickstart Scenarios from `specs/014-hybrid-search/quickstart.md` end-to-end and confirm SC-001 through SC-005 pass. **STATUS 2026-07-07**: Scenarios 1, 2, 3, 5 verified — Scenario 1 (backfill, idempotent, 780 chunks), Scenario 2 (T023), Scenario 3 (exact-term subset recall improvement — SC-002 passes, +11.4pp), Scenario 5 (access-level filtering holds with hybrid enabled, zero gm_only leakage). Scenario 4 (SC-003, judge pipeline) blocked on T020.
